...
export async function GET(context: any) {
  ...
  const items = await Promise.all(sortedPosts.map(async (blog: any) => {
    ...
    return {
      ...
      content: `<blockquote>This rendering was automatically generated and may have formatting issues. For the best experience, please visit: <a href="${postURL}">${postURL}</a></blockquote> ${content}`,
      customData: `
